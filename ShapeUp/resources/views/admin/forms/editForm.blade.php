...
<form action="{{ route('admin.edit', ['entity' => $entidad, 'category' => request()->category, 'id' => request()->id]) }}" enctype="multipart/form-data" method="POST">
    <input type="hidden" id="data-input" value="{{ json_encode($dataInput) }}" name="dataInput">
        ...
        <div class="mb-4">
            ...
            <select name="{{ $dataInput[$key] }}" id="{{ $key }}" class="block w-full py-2 px-3 bg-gray-100 rounded-lg" aria-label=".form-select-lg example">
                <!-- <option selected class="bg-purple-600">Seleccione un {{$dataInput[$key]}}</option> -->
                @foreach($input as $option)
                    <option {{ $dataInput[$key] == $option->name ? 'selected' : ''}} value="{{ $option->id }}" class="fw-bold text-purple-600">{{ $option->name }}</option>
                @endforeach
            </select>
            @else
            ...
            <input type="email" pattern="^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$" value="{{ old($dataInput[$key], $current->{$dataInput[$key]}) }}" class="block w-full py-2 px-3 bg-gray-100 rounded-lg" id="{{ $key }}" name="{{ $dataInput[$key] }}" placeholder="{{ $input }}" required>
                ...
                @elseif($input == 'Foto (introducir 1 archivo de dimensión 1:1)')
                    <input type="file" class="block w-full py-2 px-3 bg-gray-100 rounded-lg" id="{{ $key }}" name="{{ $dataInput[$key] }}" placeholder="{{ $input }}">
